...
static int numericsort(struct dirent **A, struct dirent **B, int cs) {
  ...
  while (1) {
    ...
    b += len_b;
    if (fl_ascii_isdigit(UTF_A) && fl_ascii_isdigit(UTF_B)) {
      int diff,magdiff;
      while (UTF_A == '0') {UTF_A = fl_utf8decode(a, end_a, &len_a); a += len_a;}
      while (UTF_B == '0') {UTF_B = fl_utf8decode(b, end_b, &len_b); b += len_b;}
      while (fl_ascii_isdigit(UTF_A) && UTF_A == UTF_B) {
        UTF_A = fl_utf8decode(a, end_a, &len_a); a += len_a;
        UTF_B = fl_utf8decode(b, end_b, &len_b); b += len_b;
      }
      diff = (fl_ascii_isdigit(UTF_A) && fl_ascii_isdigit(UTF_B)) ? UTF_A - UTF_B : 0;
      magdiff = 0;
      while (fl_ascii_isdigit(UTF_A)) {
        magdiff++;
        UTF_A = fl_utf8decode(a, end_a, &len_a); a += len_a;
      }
      while (fl_ascii_isdigit(UTF_B)) {
        magdiff--;
        UTF_B = fl_utf8decode(b, end_b, &len_b); b += len_b;
      }
