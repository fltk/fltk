...
/**
 ...
 is determined by `Fluid.proj.i18n_type`.
...
void Code_Writer::write_cstring(const char *s, int length) {
  ...
  for (; p < e;) {
    ...
    default:
      ...
      if (Fluid.proj.utf8_in_src && (c&0x80)) {
        if ((c&0x40)) {
...
// generated by Fast Light User Interface Designer (fluid) version %.4f\n\n";
  ...
  *b = 0;
  ...
  if (Fluid.proj.avoid_early_includes==0) {
    write_h_once("#include <FL/Fl.H>");
  }
  if (t && Fluid.proj.include_H_from_C) {
    if (to_codeview) {
      write_c("#include \"CodeView.h\"\n");
    } else if (Fluid.proj.header_file_name[0] == '.' && strchr(Fluid.proj.header_file_name.c_str(), '/') == NULL) {
      write_c("#include \"%s\"\n", fl_filename_name(t));
    } else {
      write_c("#include \"%s\"\n", Fluid.proj.header_file_name.c_str());
    }
  }
  std::string loc_include, loc_conditional;
  if (Fluid.proj.i18n_type==fld::I18n_Type::GNU) {
    loc_include = Fluid.proj.i18n_gnu_include;
    loc_conditional = Fluid.proj.i18n_gnu_conditional;
  } else {
    loc_include = Fluid.proj.i18n_pos_include;
    loc_conditional = Fluid.proj.i18n_pos_conditional;
  }
  if ((Fluid.proj.i18n_type != fld::I18n_Type::NONE) && !loc_include.empty()) {
    ...
    if (loc_include[0] != '<' && loc_include[0] != '\"')
      ...
      write_c("#%sinclude %s\n", indent(), loc_include.c_str());
    if (Fluid.proj.i18n_type == fld::I18n_Type::POSIX) {
      if (!Fluid.proj.i18n_pos_file.empty()) {
        write_c("extern nl_catd %s;\n", Fluid.proj.i18n_pos_file.c_str());
      } else {
        write_c("// Initialize I18N stuff now for menus...\n");
        write_c("#%sinclude <locale.h>\n", indent());
        write_c("static char *_locale = setlocale(LC_MESSAGES, \"\");\n");
        write_c("static nl_catd _catalog = catopen(\"%s\", 0);\n", Fluid.proj.basename().c_str());
      }
    }
    if (conditional) {
      write_c("#else\n");
      if (Fluid.proj.i18n_type == fld::I18n_Type::GNU) {
        if (!Fluid.proj.i18n_gnu_function.empty()) {
          write_c("#%sifndef %s\n", indent(), Fluid.proj.i18n_gnu_function.c_str());
          write_c("#%sdefine %s(text) text\n", indent_plus(1), Fluid.proj.i18n_gnu_function.c_str());
          write_c("#%sendif\n", indent());
        }
      }
      if (Fluid.proj.i18n_type == fld::I18n_Type::POSIX) {
    ...
    if (Fluid.proj.i18n_type == fld::I18n_Type::GNU && Fluid.proj.i18n_gnu_static_function[0]) {
      write_c("#ifndef %s\n", Fluid.proj.i18n_gnu_static_function.c_str());
      write_c("#%sdefine %s(text) text\n", indent_plus(1), Fluid.proj.i18n_gnu_static_function.c_str());
      write_c("#endif\n");
    }
  }
