...
Fl_Color lighter(Fl_Color c) { return fl_color_average(c, FL_WHITE, 0.98); }
Fl_Color  darker(Fl_Color c) { return fl_color_average(c, FL_BLACK, 0.98); }

void get_color(int index, uchar& r, uchar& g, uchar& b, int brightness) {
  Fl_Color c = palette[index % palette_size] << 8;
  for (int i=0; i>brightness; i--) c = darker(c);
  for (int i=0; i<brightness; i++) c = lighter(c);
    ...
    for (int xi = 0; xi < W; xi++) {
      ...
      for (int i=0; ; i++) {
        ...
        if (wx*wx + wy*wy > 4) {
          ...
          if (t <= 0) t = 0; else for (i=(use_colors?MAX_BRIGHTNESS-2:brightness); i--;) t*=wx;
          int c = 255 - int(254*t);
          if (use_colors) get_color(c, *p, *(p+1), *(p+2), brightness-MAX_BRIGHTNESS/2);
          else *p = c;
          break;
        }
      }
