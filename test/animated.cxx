...
static void make_images() {
  ...
  for (i = 0; i < FRAMES; i++) {
    ...
    uchar alpha = 255;
    if (i < FRAMES / 2)
      alpha = static_cast<uchar>(255 * (i / ((float) FRAMES / 2)));
    else
      alpha = static_cast<uchar>(255 * (((FRAMES / 2) - (i - FRAMES / 2)) / ((float) FRAMES / 2)));
    ...
    for (x = spherex; x < spherex + spherew; x++) {
      for (y = 20; y < 20 + spherew; y++) {
        ...
        const uchar grey = static_cast<uchar>(fill * 255);

        uchar myalpha = alpha;
        if (fill > 0.9)
          myalpha *= static_cast<uchar>((1.0f - fill) * 10);
    ...
    const int xoffset = static_cast<int>(pos * DIM);
